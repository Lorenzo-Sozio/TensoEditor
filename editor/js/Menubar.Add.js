...
function MenubarAdd(editor) {
	...
	modelsSubmenuTitle.onMouseOver(function () {
		...
		const { top, right } = modelsSubmenuTitle.dom.getBoundingClientRect();
		const { paddingTop } = getComputedStyle(this.dom);
		modelsSubmenu.setLeft(right + 'px');
		modelsSubmenu.setTop(top - parseFloat(paddingTop) + 'px');
		modelsSubmenu.setStyle('max-height', [`calc( 100vh - ${top}px )`]);
		modelsSubmenu.setDisplay('block');

	});
	modelsSubmenuTitle.onMouseOut(function () {

		modelsSubmenu.setDisplay('none');

	});
	options.add(modelsSubmenuTitle);
	...
	for (let i = 0; i < arr.length; i++) {
		let option = new UIRow();
		option.setClass('option');
		option.setTextContent(arr[i].name);
		option.onClick(function () {
			modelMan._handleModelClick(arr[i].fileName);
		});
	...
	option = new UIRow();
	option.setClass('option');
	option.setTextContent(strings.getKey('menubar/add/mesh/trapezoid'));
	option.onClick(function () {
